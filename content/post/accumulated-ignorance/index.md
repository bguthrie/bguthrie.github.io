...
Tech debt is often understood to accumulate through _taking deliberate shortcuts_: we knew we should have written tests, but we shipped without them. We knew the schema was wrong, but we didn't have time to fix it. It's a bludgeon.
...
Mitchell Hashimoto wrote recently about a practice he calls "[harness engineering](https://mitchellh.com/writing/my-ai-adoption-journey)": the idea that anytime you find an agent making a mistake, you take the time to engineer a solution so that the agent never makes that mistake again. Better AGENTS.md files, custom scripts, automated checks. It's the discipline of building upstream guardrails.
