...
[^dbas]: Shoutout to the many, many battle-scarred DBAs out there who which they still did.
...
| Decision | Historically architectural because... | Status | Why |
|---|---|---|---|
| Language choice | Rewrite cost measured in years; team skills tied to it | **Demoted** | Mechanical translation at scale now demonstrated |
| Framework choice | Lock-in through conventions, tooling, hiring | **Demoted** | Full reimplementation feasible in days |
| Module structure | Expensive to restructure; affected every developer | **Demoted** | Find call sites, move code, verify; mechanical |
| Implementation patterns | Legibility across team; embedded everywhere | **Demoted** | Instrumental value only; cheap to swap |
| Persistence strategy | Data access code spread everywhere; migration risk | **Demoted** | Full port feasible in hours to days |
| Testing architecture | Shaped what was safe to change | **Demoted** | AI can generate and restructure test suites |
| Build and CI tooling | Deeply coupled to workflow | **Demoted** | Mechanical; clear interfaces |
| Caching strategy | Performance-critical; hard to change safely | **Split** | Implementation demoted; cache *policy* still requires judgment |
| Error handling / retries | Subtle behavioral consequences across codebase | **Split** | Implementation demoted; retry *policy* still requires judgment |
| Concurrency model | Correctness and performance implications | **Split** | Scaffolding demoted; correctness *guarantees* still hard |
| Event / messaging architecture | Shapes data flow and consistency | **Split** | Implementation demoted; consistency model and event *contracts* still architectural |
| Business logic placement | Organic drift over time; hard to untangle | **Easier** | LLM + codebase search makes finding and reorganizing scattered logic tractable |
| Infrastructure / cloud provider | Vendor lock-in; proprietary APIs | **Easier** | IaC and AI accelerate migration; data gravity and proprietary dependencies remain |
| Abstraction design | Requires deep judgment about semantic levels | **Still hard** | AI can propose; human judgment still essential for long-term system health |
| Schema design / data modeling | Data has gravity; everything depends on its shape | **Still hard** | AI rewrites access code, but the data itself remains costly to reshape |
| Data migration at scale | Operational risk; downtime; correctness | **Still hard** | AI automates verification, but live migration risk unchanged |
| Service boundaries / API contracts | Organizational and contractual; downstream impact | **Still hard** | Human coordination is the bottleneck, not code |
| Multi-tenancy model | Data isolation, security, scaling | **Still hard** | Deeply intertwined with data model and trust guarantees |
| Auth model | Security-critical; regulatory implications | **Still hard** | Wrong choice is a breach; regulatory lock-in |
| User trust / security posture | Irreversible when broken | **Still hard** | Can't refactor out of an exploited vulnerability |
| Regulatory / compliance | Legal lock-in; audit requirements | **Still hard** | No AI impact on legal obligations themselves |
| Observability / SLO design | Historically undervalued | **Elevated** | If trust is architectural, detecting trust violations is too |
| Verification harnesses | Historically undervalued | **Elevated** | Behavioral verification is now the bottleneck |
| Business strategy as architecture | Historically the architect's blind spot | **Elevated** | With code-level work automated, the strategic gap is the whole job |

**Almost every decision has a mechanical implementation layer that AI has demoted, and a judgment, contract, or trust layer that remains important.** However, some judgments become dramatically easier to reverse. The things that are genuinely still hard are generally hard because they involve coordination
with humans or irreversible real-world consequences, not because the judgment is difficult to express or reverse.
